...
void draw() {
  ...
  for (int i=0; i<totalCars; i++) {
    ...
    //if selected, use brighter colours
    if (i == selectedCar) {
...
void mousePressed() {
  //pick the nearest car in screen space
  selectedCar=-1;
  float bestDist = 999999;
  for (int i = 0; i < totalCars; i++) {
    //approximate 2D picking using car center on screen
    float screenPX = width / 2 + carX[i];
    float screenPY = height / 2 + carY[i];

    float d = dist(mouseX, mouseY, screenPX, screenPY);

    //clickable radius changes with car size
