...
//mouse offset so the car does not jump to its center when dragged
float dragOffsetX = 0;
float dragOffsetY = 0;
...
void mousePressed() {
  ...
  for (int i = 0; i < totalCars; i++) {
    //approximate picking in screen space
...
void mouseDragged() {
  //move selected car in 2D space only (x and y)
  if (selectedCar != -1) {
